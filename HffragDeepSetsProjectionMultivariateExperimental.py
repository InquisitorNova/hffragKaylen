...
class TimeDistributedResidualUnits(keras.layers.Layer):
    def __init__(self, d_model, dropout = 0.01):
        ...
        self.seq = tf.keras.Sequential([
            layers.TimeDistributed(layers.Dense(d_model, activation = "gelu",  kernel_initializer = "he_normal")),
            layers.TimeDistributed(layers.Dense(d_model, activation = "gelu",  kernel_initializer = "he_normal")),
            MCDropout(dropout)
...
class ResidualUnits(keras.layers.Layer):
    def __init__(self, d_model, dropout = 0.01):
        super().__init__()
        self.seq = tf.keras.Sequential([
            layers.Dense(d_model, activation = "gelu",  kernel_initializer = "he_normal"),
            layers.Dense(d_model, activation = "gelu",  kernel_initializer = "he_normal"),
            MCDropout(dropout)
        ])
        self.add = tf.keras.layers.Add()
...
def DeepSetsProjection(track_layers, jet_layers, b_jet_layers, n_targets, n_targets_classification, n_features, regularizer_strength, dropout = 0.01):
    ...
    outputs_tracks = layers.TimeDistributed(layers.Dense(track_layers[0], kernel_initializer="he_normal"))(outputs_tracks)
    outputs_tracks = layers.BatchNormalization()(outputs_tracks)
    outputs_tracks = layers.LeakyReLU()(outputs_tracks)
    counter = 0

    #The track network extracts information from the track features.
    for nodes in track_layers[1:-1]:
        outputs_tracks = layers.TimeDistributed(layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength)))(outputs_tracks)
        outputs_tracks = layers.BatchNormalization()(outputs_tracks)
        outputs_tracks = layers.LeakyReLU()(outputs_tracks)
        y = layers.TimeDistributed(layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength)))(outputs_tracks)
        y = layers.BatchNormalization()(y)
        y = layers.LeakyReLU()(y)
        y = layers.TimeDistributed(layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength)))(y)
        y = layers.BatchNormalization()(y)
        y = layers.LeakyReLU()(y)
        y = layers.TimeDistributed(layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength)))(y)
        y = layers.BatchNormalization()(y)
        y = layers.LeakyReLU()(y)
        u = MCDropout(dropout)(y)
        outputs_tracks = layers.Add()([outputs_tracks, u])
        if counter > 0:
            outputs_tracks = layers.Add()([outputs_tracks, output_tracks_previous])
        outputs_tracks = layers.BatchNormalization()(outputs_tracks)
        output_tracks_previous = outputs_tracks
        counter +=1
    ...
    for nodes in b_jet_layers[:-1]:
        outputs_jets = layers.Dense(nodes, kernel_initializer= "he_normal")(outputs_jets) # We use relu and the corresponding he_normal for the activation function and bias initializer
        outputs_jets = layers.BatchNormalization()(outputs_jets)
        outputs_jets = layers.LeakyReLU()(outputs_jets)
    outputs_jets = layers.Dense(b_jet_layers[-1], kernel_initializer="he_normal")(outputs_jets) # Apply softmax to ouput the results of the track neural network as probabilities
    ...
    for nodes in jet_layers[:-1]:
        outputs = layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength))(outputs)
        outputs = layers.BatchNormalization()(outputs)
        outputs = layers.LeakyReLU()(outputs)
        y = layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength))(outputs)
        y = layers.BatchNormalization()(y)
        y = layers.LeakyReLU()(y)
        y = layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength))(y)
        y = layers.BatchNormalization()(y)
        y = layers.LeakyReLU()(y)
        y = layers.Dense(nodes, kernel_initializer="he_normal", kernel_regularizer=regularizers.l1_l2(regularizer_strength))(y)
        y = layers.BatchNormalization()(y)
        y = layers.LeakyReLU()(y)
        u = MCDropout(dropout)(y)
        outputs = layers.Add()([outputs, u])
        if counter > 0:
            outputs = layers.Add()([outputs, outputs_previous])
        outputs = layers.BatchNormalization()(outputs)
        outputs_previous = outputs
        counter+=1
