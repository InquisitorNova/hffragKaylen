"""
*Filename: hffragTransformerJetMultivariateSVFinder
...
*This transformer is regresses the bhadron features and classifies the bhadron masses. This transformer consists of an
*particle and jet embedder which creates embedding for the tracks and jets. It consists of an encoder consisting of 
*several self attention layers followed by a feedforward neural network to take the features derived
...
#Combining the b_jet and derived jet features the bhadsNet regress the bhadron targets and classify the bhadron masses.
def bhadsNet(n_features,n_targets_classification, d_model, n_targets, jet_layers):
        ...
        outputs_Target_jets_mass = layers.Dense(256, activation='softmax')(outputs_target_jets)
        outputs_Target_jets_mass = layers.Dense(256, activation='softmax')(outputs_Target_jets_mass)
        ...
        Outputs = layers.concatenate([outputs_target_jets,outputs_variances_jets, outputs_covariances_jets], name = "MultivariateLoss")

        Model = keras.Model(inputs = [b_jets,jets_derived], outputs = [Outputs, outputs_Target_jets_mass])
...
class B_Jet_Layer(keras.layers.Layer):
    def __init__(self, n_targets, n_targets_classification, dff, jet_layers,  n_features,d_model, MASKVAL=-999):
        super().__init__()
        ...
        self.JetNet = bhadsNet(self.n_features, self.n_targets_classification, self.d_model, self.n_targets,self.jet_layers)
    
    def call(self,x, context):
        x,y = self.JetNet([x, context])
        return x,y
...
#features and classify the bhadron masses.
class hffragTransformer(tf.keras.Model):
    def __init__(self, *,track_layers, n_targets_classifications, jet_layers, d_model, num_heads, dff, n_targets, n_features, MASKVAL = -999, dropout_rate = 0.1):
        super().__init__()
        ...
        self.jet_layer = B_Jet_Layer(n_targets = n_targets, n_targets_classification= n_targets_classifications, n_features=n_features, dff = dff, d_model= d_model, jet_layers = jet_layers, MASKVAL=MASKVAL)
    ...
    def call(self, inputs):
        ...
        MultivariateOutput, MassOutput = self.jet_layer(Jets,Embedding)

        return MultivariateOutput, MassOutput
