...
* monte carlo dropout, skip connections.
Date: 16/02/2023
...
#to regress the bhadron features

def DeepSetsProjection(track_layers, jet_layers, b_jet_layers, n_targets, regularizer_strength, n_features, Dropout_rate = 0.0001):
    ...
    Outputs_Multivariate = layers.concatenate([outputs_Target_jets,outputs_variances_jets, outputs_covariances_jets])

    Outputs_Categorical = layers.concatenate([outputs_Target_jets,outputs_variances_jets, outputs_covariances_jets])

    Outputs_Categorical = layers.Dense(nodes, activation = "gelu", kernel_initializer="he_normal", kernel_regularizer= regularizers.l1_l2(regularizer_strength))(Outputs_Categorical)
    
    Model = keras.Model(inputs = [tracks,jets], outputs = [Outputs_Multivariate, Outputs_Categorical])
