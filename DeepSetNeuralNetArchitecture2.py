...
def Match_Reconstructed(true_jets, reconstructed_jets):
    ...
    return np.sqrt(delta_phis**2 + delta_etas**2) < 0.3

# Find the associated tracks for each jet
def Match_Tracks(true_jets, tracks):
    ...
    delta_etas = true_jet_eta - tracks_eta
    delta_phis = np.abs(true_jet_phi - tracks_phi)
...
def Match_Electrons(true_jets, electrons):
    ...
    delta_etas = true_jet_eta - electron_eta
    delta_phis = np.abs(true_jet_phi - electron_phi)
...
#Finds the associated muons for each jet
def Match_Muons(true_jets, muons):
    "Used to determine if a set of muons belong to a particular set of muons"
    ...
    delta_etas = true_jet_eta - muon_eta
    delta_phis = np.abs(true_jet_phi - muon_phi)
