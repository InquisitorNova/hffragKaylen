...
def bhadsNet(n_features,d_model, activation_func_0, Initializer_0, regularizer_strength_0, n_targets, Num_jet_layers):
        b_jets = layers.Input(shape = (d_model))
        ...
        for _ in range(Num_jet_layers): #Repeat of the track neural network without the need for the timedistributed layers
            outputs = layers.Dense(d_model,activation = activation_func_0, kernel_initializer= Initializer_0, kernel_regularizer = regularizers.l1_l2(regularizer_strength_0))(output_bjets)
...
class B_Jet_Layer(keras.layers.Layer):
    def __init__(self, n_targets, dff, activation_func_0, Initializer_0, regularizer_strength_0, jet_layers, n_features,d_model, MASKVAL=-999):
        ...
        self.JetNet = bhadsNet(n_features=self.n_features, d_model= self.d_model, activation_func_0= self.activation_func, Initializer_0=self.Initializer, regularizer_strength_0= self.regularizer, n_targets= self.n_targets, Num_jet_layers= self.jet_layers)
...
class FeedForward(tf.keras.layers.Layer):
    def __init__(self,d_model, activation_func_3, Initializer_func_3, regularizer_func_3, dff, dropout):
        super().__init__()
        self.seq = tf.keras.Sequential([
            tf.keras.layers.Dense(dff,activation = activation_func_3,  kernel_initializer = Initializer_func_3, kernel_regularizer = regularizers.l1_l2(regularizer_func_3)),
            tf.keras.layers.Dense(d_model, activation = activation_func_3,  kernel_initializer = Initializer_func_3, kernel_regularizer = regularizers.l1_l2(regularizer_func_3)),
            tf.keras.layers.Dense(d_model, activation = activation_func_3,  kernel_initializer = Initializer_func_3, kernel_regularizer = regularizers.l1_l2(regularizer_func_3)),
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, track_layers, activation_func_1, Initializer_1, regularizer_strength_1, activation_func_3, Initializer_func_3, regularizer_func_3, d_model, num_heads, dff,MASKVAL = -999, dropout_rate = 0.1):
        super().__init__()
        ...
        self.encoder_layers = [
            ...
            regularizer_func_3 = regularizer_func_3,
            dff = dff,
...
class hffragTransformer(tf.keras.Model):
    def __init__(self, Num_tracks_layer, Num_jets_layer, activation_func_0, Initializer_value_0, regularizer_value_0, activation_func_1, Initializer_value_1, regularizer_value_1,activation_func_2, Initializer_value_2, regularizer_value_2, activation_func_3, Initializer_value_3, regularizer_value_3,  d_model, Num_Heads, dff, n_targets, n_features, MASKVAL = -999, dropout_rate = 0.1):
        super().__init__()

        self.encoder_tracks = Encoder(track_layers=Num_tracks_layer, activation_func_1=activation_func_1, Initializer_1= Initializer_value_1, regularizer_strength_1= regularizer_value_1, activation_func_3= activation_func_3, Initializer_func_3=Initializer_value_3, regularizer_func_3= regularizer_value_3, d_model = d_model, num_heads=Num_Heads, dff = dff, MASKVAL = MASKVAL, dropout_rate= dropout_rate)

        self.encoder_jets = JetEmbedder(d_model = d_model, activation_func_2= activation_func_2, Initializer_2= Initializer_value_2, regularizer_strength_2= regularizer_value_2, MASKVAL= MASKVAL)

        self.jet_layer = B_Jet_Layer(n_targets=n_targets, dff = dff, activation_func_0= activation_func_0, Initializer_0= Initializer_value_0, regularizer_strength_0= regularizer_value_0, jet_layers = Num_jets_layer, n_features= n_features, d_model=d_model, MASKVAL=MASKVAL)
...
def build_model(hp):
    ...
    dff = hp.Choice('dff',[16,32,64,128,256,512])
    d_model = hp.Choice('d_model',[16,32,64,128,256,512])
        
    activation_func_0 = hp.Choice('act_func_0',["relu","elu","selu", "gelu"])
    activation_func_1 = hp.Choice('act_func_1',["relu","elu","selu", "gelu"])
    activation_func_2 = hp.Choice('act_func_2',["relu","elu","selu", "gelu"])
    activation_func_3 = hp.Choice('act_func_3',["relu","elu","selu", "gelu"])
    ...
    Initializer_value_0 = hp.Choice('Initalizers_0', ["he_normal", "lecun_normal","glorot_uniform"])
    Initializer_value_1 = hp.Choice('Initalizers_1', ["he_normal", "lecun_normal","glorot_uniform"])
    Initializer_value_2 = hp.Choice('Initalizers_2', ["he_normal", "lecun_normal","glorot_uniform"])
    Initializer_value_3 = hp.Choice('Initalizers_3', ["he_normal", "lecun_normal","glorot_uniform"])

    regularizer_value_0 = hp.Float("regularizer_0", 1e-6,1e-1)
    regularizer_value_1 = hp.Float("regularizer_1", 1e-6,1e-1)
    regularizer_value_2 = hp.Float("regularizer_2", 1e-6,1e-1)
    regularizer_value_3 = hp.Float("regularizer_3", 1e-6,1e-1)

    Num_Heads = hp.Int("Num_Heads",1,10)

    beta_1 = hp.Float("beta_1", 0.5,0.999)
    beta_2 = hp.Float("beta_2", 0.5,0.999)
    Learning_rate = hp.Float("Learning_rate", 1e-6,1e-2)
    weight_decay = hp.Float("Weight_Decay", 0.1,1.0)

    #Set the number of targets being explored
    n_targets = 4
    n_features = np.shape(b_jets_input)[1]
    
    ParticleTransformer = hffragTransformer(Num_tracks_layer, Num_jets_layer, activation_func_0, Initializer_value_0, regularizer_value_0, activation_func_1, Initializer_value_1, regularizer_value_1,activation_func_2, Initializer_value_2, regularizer_value_2, activation_func_3, Initializer_value_3, regularizer_value_3,  d_model, Num_Heads, dff, n_targets, n_features, MASKVAL, dropout_rate)

    ParticleTransformer.compile(
        optimizer= tf.keras.optimizers.Nadam(Learning_rate, beta_1= beta_1, beta_2 = beta_2, weight_decay= weight_decay, clipnorm = 10),
...
Tuner = kt.BayesianOptimization(
  ...
  project_name = "TransformerProjector",
)
