...
def DeepSetsProjection(track_layers, jet_layers, b_jet_layers, n_targets, regularizer_strength, n_features, Dropout_rate = 0.1):
    ...
    for nodes in track_layers[:-1]:
        ...
        outputs_tracks = MCDropout(Dropout_rate)(outputs_tracks)
        outputs_tracks = layers.LayerNormalization()(outputs_tracks)
    ...
    for nodes in b_jet_layers[:-1]:
        ...
        outputs_jets = MCDropout(Dropout_rate)(outputs_jets)
    ...
    Model = keras.Model(inputs = [tracks,jets], outputs = outputs)
